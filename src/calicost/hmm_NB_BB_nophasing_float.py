        ...
        for i in np.arange(n_states):
            for s in np.arange(n_spots):
                ...
                if len(idx_nonzero_rdr) > 0:
                    ...
                    log_emission_rdr[i, idx_nonzero_rdr, s] = nbinom_logpmf_float(X[idx_nonzero_rdr, 0, s], n, p)
        ...
        for r in range(max_iter):
            ...
            if tumor_prop is None:
                log_emission_rdr, log_emission_baf = (
                    hmm_nophasing_float.compute_emission_probability_nb_betabinom(
                ...
                if ((not log_gamma is None) or (r > 0)) and ("m" in self.params):
                    ...
                    log_emission_rdr, log_emission_baf = (
                        hmm_nophasing_float.compute_emission_probability_nb_betabinom_mix(
                            X,
                            base_nb_mean,
                            log_mu,
                            alphas,
            ...
            log_alpha = hmm_nophasing_float.forward_lattice(
                lengths,
                log_transmat,
                log_startprob,
                log_emission,
            ...
            log_beta = hmm_nophasing_float.backward_lattice(
                lengths,
                log_transmat,
                log_startprob,
                log_emission,
