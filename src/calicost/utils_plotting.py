...
def plot_individual_spots_in_space_categorical(coords, assignment, single_tumor_prop=None, sample_list=None, sample_ids=None, base_width=4, base_height=3, palette="Set2", add_grey=True):
    # combine coordinates across samples
    shifted_coords = copy.copy(coords)
    if not (sample_ids is None):
        x_offset = 0
    ...
    # number of clones and samples
    final_clone_ids = assignment.categories
    n_final_clones = len(final_clone_ids)
    n_samples = 1 if sample_list is None else len(sample_list)
