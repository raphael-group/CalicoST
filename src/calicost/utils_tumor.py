...
def get_tumor_weight_v1(sample_lengths, tumor_prop, log_mu, logmu_shift):
...
def get_tumor_weight(sample_lengths, tumor_prop, log_mu, logmu_shift):
    ...
    for c in range(len(sample_lengths)):
        ...
        mu = np.exp(log_mu)
        mu_norm = np.exp(logmu_shift[c, :])
        
        normed_mu = (mu / mu_norm)[:, None, :]        
        result = (
            range_tumor_prop * normed_mu
            / (range_tumor_prop * normed_mu + 1. - range_tumor_prop)
        )
